...
void testUploadTrack(prc::Client& client, prc::id_t accountId, prc::id_t feedId)
{
    prc::Track track;
    ...
    track.extId = sole::uuid4().str();
    track.begin = prism::test::generateTimestamp();
    track.end = prism::test::generateTimestamp();
    track.objectId = sole::uuid4().str();
    track.frameWidth = 1280;
    track.frameHeight = 720;
    track.points = points;
...
int main(int argc, char** argv)
{
    ...
    std::string blob_id = sole::uuid4().str();
    ...
    for(;;)
    {
        ...
        if (!motion && was_motion)
            blob_id = sole::uuid4().str(); // Generate new id
        ...
        if (needUpdateData)
        {
            ...
            if (enableBackground)
            {
                ...
                prc::Background bg;
                bg.extId = sole::uuid4().str(); // regenerate here
